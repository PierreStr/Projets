...
# 
# Return : the solution find by Gurobi for the model "OptVax2-LP"
def Solve_OptVax2_LP(file_name, C_km, time_out, bool_print, warm_start = False):
    ...
    w = model.addVars(M, I + J, I + J, vtype=grb.GRB.CONTINUOUS, lb=0, ub=1, name="w")  # Movements of MMTs
    u = model.addVars(M, I + J, vtype=grb.GRB.CONTINUOUS, lb=0, name="u")  # Locality's order of passage 
    x = model.addVars(I, vtype=grb.GRB.CONTINUOUS, lb=0, ub=1, name="x")  # Opening VC
    y = model.addVars(I, J, vtype=grb.GRB.CONTINUOUS, lb=0, ub=1, name="y")  # Localities covered by VC
    v = model.addVars(M, vtype=grb.GRB.CONTINUOUS, lb=0, ub=1, name="v")  # Opening route
    t = model.addVars(M, J, vtype=grb.GRB.CONTINUOUS, lb=0, ub=1, name="t")  # Localities covered by MMT
    ...
    # Valid inequality
    model.addConstr(grb.quicksum(v[m] for m in range(M)) >= (grb.quicksum(q[j]*grb.quicksum(t[m, j] for m in range(M)) for j in range(J)))/Q)
